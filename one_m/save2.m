...
gradx=tmp1(x-7:x+8,y-7:y+8);
%tmp2=r(x-8:x+9,y-7:y+8);
tmp2=conv2(r,gradient','same');
grady=tmp2(x-7:x+8,y-7:y+8);
magr=zeros(16,16);
orir=zeros(16,16);
for i=1:16
    for j=1:16
...
for row=0:3
    for col=0:3
        ...
        if index==8
            off=hist(7)/(hist(7)+hist(1));
        elseif index==1
            off=hist(8)/(hist(2)+hist(8));
        else
            off=hist(index-1)/(hist(index+1)+hist(index-1));
        end
            
        angle=(index-1)*pi/4+(pi/4)*off;
